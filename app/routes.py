...
from app.utils import calculate_statistics, get_grade_distribution, get_student_ranking, get_course_statistics, validate_score
...
def api_students():
    ...
    return jsonify([{'id': s.id, 'student_id': s.student_id, 'name': s.name, 'class_name': s.class_name, 'email': s.email} for s in students])
...
def api_student_detail(student_id):
    ...
    if request.method == 'GET':
        return jsonify({'id': student.id, 'student_id': student.student_id, 'name': student.name, 'class_name': student.class_name, 'email': student.email})
...
def api_courses():
    ...
    return jsonify([{'id': c.id, 'code': c.code, 'name': c.name, 'description': c.description} for c in courses])
...
def api_course_detail(course_id):
    ...
    if request.method == 'GET':
        return jsonify({'id': course.id, 'code': course.code, 'name': course.name, 'description': course.description})
    elif request.method == 'PUT':
        data = request.get_json() or request.form
...
def api_grades():
    ...
    return jsonify([{'id': g.id, 'student_id': g.student_id, 'course_id': g.course_id, 'score': g.score, 'exam_type': g.exam_type, 'created_at': g.created_at.isoformat()} for g in grades])
...
def api_grade_detail(grade_id):
    ...
    if request.method == 'GET':
        return jsonify({'id': grade.id, 'student_id': grade.student_id, 'course_id': grade.course_id, 'score': grade.score, 'exam_type': grade.exam_type, 'created_at': grade.created_at.isoformat()})
    elif request.method == 'PUT':
        data = request.get_json() or request.form
