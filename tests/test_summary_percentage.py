...
from app.models import User, Student, Course, Grade, ExamScheme, GradeBandSet
...
def login_admin(client, app):
    with app.app_context():
        ...
        db.session.add(ExamScheme(exam_type='regular', subject_code='TOTAL', subject_name='总分', max_score=300))
        db.session.commit()
        g = Grade(student=s, course=c_total, score=270, exam_type='regular', exam_name='期末')
        db.session.add(g)
        # 发布一个版本供展示
        db.session.add(GradeBandSet(exam_name='期末', version=1, status='published', rules_json='[]'))
        db.session.commit()
