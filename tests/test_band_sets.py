...
def test_create_publish_rollback(client, app):
    ...
    # 在草稿上保存规则（快照）
    items = [
        {"subject_code": "CN", "method": "range", "a_min": 85, "b_min": 70, "c_min": 50, "d_min": 30},
        {"subject_code": "TOTAL", "method": "percentile", "a_pct": 30, "b_pct": 30, "c_pct": 20, "d_pct": 10, "e_pct": 10},
    ]
    r = client.put('/api/grade-bands/bulk', json={'exam_name': '期中', 'rule_set_id': draft['id'], 'items': items})
    assert r.status_code == 200
